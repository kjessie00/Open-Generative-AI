...
본 §11 은 본 shorts 도메인 명세가 어떤 외부 호출 surface 만을 통해 라이브 image / video / research 호출을 수행할 수 있는지를 owner 결정 으로 고정한다. 본 §11 은 본 §10 의 외부 실행 금지 와 본 §2 의 사전 라이브러리 호출 정의 의 두 가지 운영 원칙을 surface 화이트리스트 영역 으로 확장한 것이다.

본 shorts 도메인의 외부 호출 surface 는 다음 3 개 skill 로 단일화된다. 첫째, shorts 의 reference / scene 이미지 생성은 deepsearch-team skill 의 `dst image` 또는 `dst agent` 모드 (goldpure369) 만 사용한다. 둘째, shorts 의 비디오 생성은 dreamina-video-cli skill 의 `dreamina` CLI (seedance2.0mini / fast / 2.0) 또는 google-labs-flow-auto skill 의 `run_v3_gen.py` 둘 중 owner 가 작업별로 지정한 하나만 사용한다. 셋째, 위 두 도메인 외 어떤 surface 도 라이브 호출에 사용되지 않는다. 금지 surface 목록은 MuAPI / 호스팅 SaaS API / OpenAI 직접 호출 / Anthropic 직접 호출 / Gemini API 직접 호출 / Replicate / Runway / Pika / Sora / Veo 직접 호출 / 기타 임의의 image 또는 video 생성 endpoint 다.

본 결정의 운영 의미는 본 §10 의 외부 실행 금지 가 본 §11 의 surface lock 으로 추가로 강화된다는 점이다. electron 측 executor 화이트리스트 가 화이트리스트 외 surface 호출을 명령 미리보기 모드로 강제 다운그레이드한다. 본 shorts 도메인의 어떤 패널이 실수로 잘못된 endpoint 를 호출하려 해도 이 화이트리스트가 호출을 차단한다.
