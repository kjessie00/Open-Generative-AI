...
## 후속 renderer path provenance 보완

Root 감사에서 초기 구현의 `setConfig(config)` public bridge와 renderer path
인자형 folder selection이 발견되었다. 따라서 payload root와 config root의
문자열 일치만으로는 root의 출처를 보장하지 못했다. 이 gap과 보완 결과는
`docs/ui_integration/30_renderer_path_provenance.md`가 현재 기준이다.

후속 구현은 public `setConfig` bridge/handler를 제거하고, native dialog 결과와
configured parent의 immediate real child만 main-owned config에 기록한다. 계획
파일의 기존 exact allowlist, content cap, symlink, temp/atomic 경계는 그대로
유지한다.

독립 인수는 pending이 아니라 `BLOCK`이다. 첫 non-root 독립 verifier와 정확히
한 번의 fallback verifier가 모두 코드 판정 전에 cybersecurity classifier에서
실패하여 독립적인 코드 verdict가 생성되지 않았다. 자동 verifier를 더 호출하지
않으며 root도 이 보안 변경을 최종 security acceptance로 승격하지 않는다.
