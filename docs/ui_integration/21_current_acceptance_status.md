...
| AC5 실제 GUI | PARTIAL PASS | 19-method/11개 한국어 menu/4 viewport/두 root 비-native 증거와 후속 Blob-only metadata/save/export/full quit-relaunch 및 별도 OS 창 캡처 PASS; current native selection과 actual mobile keyboard-only는 BLOCK |
...
| AC7 자동 검증 | VERIFIED (명시된 독립 gaps 제외) | receiver 회귀 포함 blob focused 22/22, network-denied 전체 148/148, lint, build 48 modules와 수정 후 actual Electron G3 회차 PASS |
...
1. G3 blob helper의 Chromium `atob` receiver 수정과 실제 Electron
   metadata/save/export/full quit-relaunch 및 별도 OS 창 캡처는 PASS다. 이 결과는
   격리 fixture runtime 인수이며 실제 production output quality나 승격은 아니다.
