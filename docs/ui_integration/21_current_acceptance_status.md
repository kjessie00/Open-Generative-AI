...
계획 파일 쓰기 IPC도 최소권한으로 하드닝했다. 현재 설정된 non-symlink
production root와 정확히 일치하는 요청에서 UI가 사용하는 세 산출물 패턴만
허용하며, 1 MiB content cap, parent/leaf symlink 차단, same-directory
exclusive/no-follow temp, flush/close, atomic rename과 실패 cleanup을 적용했다.
실제 temp filesystem 공격 회귀와 renderer reject 복구는 PASS다. 다만 이
security-significant 변경의 immutable commit 독립 검증은 executor commit 뒤
별도 read-only verifier가 수행해야 하므로 최종 보안 인수는 그 결과 전까지
pending이다. 상세 증거는 `docs/ui_integration/29_planning_write_security.md`를
따른다.
...
| AC2 Electron 보안 | VERIFIED (planning-write 독립 검증 pending) | 외부 navigation deny-by-default, active provider/bridge 최소권한; planning write의 exact allowlist/symlink/content/atomic-write executor 회귀 PASS |
| AC3 renderer/main 경계 | VERIFIED (planning-write 독립 검증 pending) | renderer global은 `window.filmPipeline`만 노출하고 main이 file/preview 경계를 소유; configured root 일치와 한국어 reject 복구를 강제; `window.localAI` undefined |
| AC4 side-effect 차단 | VERIFIED (code/test) | live generation/upload는 연결하지 않았고 command preview만 허용 |
| AC5 실제 GUI | PARTIAL PASS | 실제 window/preload/11-tab/fixture+첫 root native/state/blocked preview/trusted copy와 한국어 4-viewport/AX/axe/focus PASS; 상대/remote media source 차단과 mobile select DOM change 회귀 PASS; 두 번째 root native selection, 수정 후 console 및 실제 keyboard-only 증거는 BLOCK |
...
| AC7 자동 검증 | VERIFIED (명시된 GUI gaps 제외) | planning-write 하드닝 후 network-denied 전체 89/89, lint, build 41 modules, diff check PASS; 기존 4-viewport 실제 GUI runtime 증거는 별도 보존 |
...
- network-denied 전체 테스트: 89/89 PASS
- lint: PASS
...
7. planning-write security commit은 별도 non-root read-only verifier가 immutable
   commit/tree와 실제 공격 회귀를 확인한 뒤 최종 보안 인수로 승격한다.
