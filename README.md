...
- G3 인간 검토 작업대: 비승격 초안 저장/내보내기는 구현됐지만 현재 실제 미리보기는 CSP가 `data:` 동영상을 거부해 BLOCK
- 실제 production 후보: partial/unknown 상태로 안전하게 차단
- 실제 Electron 창: 현재 19-method bridge, 11개 한글 메뉴, 4개 반응형 화면과 두 승인 production의 비-native 읽기 PASS
...
8. `클립 QA`에서 `shot_manifest.json`으로 입증된 샷 별칭, 선택 테이크의
   ...
   - 현재 build에서는 후보 미리보기가 CSP에 막히므로 `로컬 검토 초안 저장`과
     `canonical 형태로 초안 내보내기`를 production 인수된 기능으로 사용하지 않습니다.
     CSP/preview transport 수정과 실제 Electron 재검증 전에는 자동 테스트 상태로만 봅니다.
   - 결과는 Electron `userData`의 비공개 폴더에만 저장되는
     `초안/비승격` 패킷입니다. production의 `selected_takes.json`은 바뀌지 않습니다.
...
- G3 renderer는 production 경로를 전달하지 않고 main이 만든 opaque 후보 token만 사용합니다. Main의 32 MiB 이하 bounded base64 응답은 실제 IPC까지 확인됐지만, 현재 renderer CSP가 `data:` 미디어 재생을 차단한다는 runtime blocker가 남아 있습니다.
...
2026-07-14 G3 focused 검증은 29/29, 전체 network-denied suite는 141/141,
lint와 Vite build 47 modules가 PASS했습니다. 상세 결과는
...
[`36_current_electron_runtime_acceptance.md`](docs/ui_integration/36_current_electron_runtime_acceptance.md)에 기록합니다.
...
현재 build의 실제 창에서 19-method preload, 11개 한글 메뉴, fixture 상태 복원,
두 승인 production의 비-native 읽기, 4개 반응형 화면, 외부 요청 0건과 실행 가능한
generation/submit/upload control 0건은 확인했습니다. 다음 항목은 아직 남아 있습니다.

- G3 preview transport/CSP 계약 수정
- 실제 preview metadata-ready 뒤 save/export와 mode `0700`/`0600` 파일 증거
- G3 full quit/relaunch restore
...
- production 승격 importer/CAS, selected-range render/fresh probe 계약과 실제 output-quality 승인

따라서 현재 앱은 production을 읽고 상태를 복원·감사하는 작업대로 사용할 수 있지만,
G3 동영상 확인부터 저장·내보내기까지 완결되는 production 검토 작업대로는 아직 최종 승인되지 않았습니다.
