...
- G3 인간 검토 작업대: 승인된 로컬 후보를 샷·비트에 연결하고 비승격 초안으로 저장/내보내기 구현
- 실제 production 후보: partial/unknown 상태로 안전하게 차단
- 실제 Electron 창과 native folder dialog의 과거 시각 검증: 부분 통과; 현재 19-method bridge와 G3 화면의 실제 GUI 재검증은 남음
...
8. `클립 QA`에서 `shot_manifest.json`으로 입증된 샷 별칭, 선택 테이크의
   ...
   - `로컬 검토 초안 저장` 후 모든 검증이 통과하면
     `canonical 형태로 초안 내보내기`를 누릅니다.
...
- G3 renderer는 production 경로를 전달하지 않고 main이 만든 opaque 후보 token만 사용합니다. 후보 미리보기는 허용된 로컬 동영상의 32 MiB 이하 bounded base64 응답입니다.
